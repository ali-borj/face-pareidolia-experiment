...
const STATE = {
  ...
  facePool:        [],       // globally shuffled face stimuli (no repeats across blocks)
  nonfacePool:     [],       // globally shuffled nonface stimuli
  facePoolIdx:     0,
  nonfacePoolIdx:  0,
...
// Call once after stimuli are loaded. Builds globally shuffled pools so that
// no image repeats across blocks within a session.
function prepareGlobalStimuliPools() {
  STATE.facePool    = seededShuffle(STATE.stimuli.filter(s => s.type === 'face'));
  STATE.nonfacePool = seededShuffle(STATE.stimuli.filter(s => s.type === 'nonface'));
  STATE.facePoolIdx    = 0;
  STATE.nonfacePoolIdx = 0;
}
...
function buildBlockTrialSequence(blockDef) {
  ...
  // Draw next slice from the global (non-repeating) pools
  const faceSlice    = STATE.facePool.slice(STATE.facePoolIdx, STATE.facePoolIdx + faceCount);
  STATE.facePoolIdx += faceCount;

  const nonfaceSlice    = STATE.nonfacePool.slice(STATE.nonfacePoolIdx, STATE.nonfacePoolIdx + nonfaceCount);
  STATE.nonfacePoolIdx += nonfaceCount;
...
const DataCollector = {
  buildRecord(stimDef, response, rt_ms, blockDef, trialIndex, globalTrialNum, phase) {
    ...
    return {
      ...
      phase:           phase,
      difficulty:      stimDef.difficulty  !== undefined ? stimDef.difficulty  : null,
      image_gender:    stimDef.gender      !== undefined ? stimDef.gender      : null,
    };
  },
};
...
const CSV_COLS = [
  ...
  'trial_type','stimulus_id','facelikeness','difficulty','image_gender',
...
// Mobile touch buttons — fire synthetic keydown events
function fireMobileKey(key) {
  document.dispatchEvent(new KeyboardEvent('keydown', { key, bubbles: true, cancelable: true }));
}

document.getElementById('btn-mobile-f').addEventListener('touchstart', (e) => {
  e.preventDefault();
  fireMobileKey('f');
}, { passive: false });

document.getElementById('btn-mobile-j').addEventListener('touchstart', (e) => {
  e.preventDefault();
  fireMobileKey('j');
}, { passive: false });
...
const FlowController = {
  ...
  async run() {
    ...
    STATE.demographics = await this.runDemographics();
  ...
  async runEnd() {
    ...
    if (url) {
      ...
      saveStatus.textContent = 'Data saved to server successfully.';
    } else {
